...
/**
 * OptionWheel — curved scroll/drag selector from React Bits (reactbits.dev),
 * ported to TSX for this project. Logic is unchanged from the upstream
 * JavaScript variant except for one addition: an optional *controlled*
 * `selectedIndex` prop. The app shell nav uses it to keep the wheel in sync
 * with the current route (e.g. drilling into a lake highlights "Live Network")
 * without the wheel's own onChange firing a redundant navigation.
 */
...
export interface OptionWheelProps {
  ...
  /** Controlled selection. When set and changed, drives the wheel without
   *  firing onChange (prevents nav → wheel → nav feedback loops). */
...
const OptionWheel = ({
  ...
  const initial = selectedIndex ?? defaultSelected;
  ...
  const runFrame = useCallback((now: number) => {
    ...
    lastRef.current = now;
    const cfg = cfgRef.current as unknown as {
      count: number; items: string[]; rowH: number; curve: number; tilt: number;
      blur: number; fade: number; minOpacity: number; side: string; loop: boolean; smoothing: number;
    };
  ...
  // Controlled sync: move to an externally-driven index WITHOUT firing onChange.
  const syncTo = useCallback(
    (value: number) => {
      const cfg = cfgRef.current as { count: number };
      const idx = ((Math.round(value) % cfg.count) + cfg.count) % cfg.count;
  ...
  // React to controlled selectedIndex changes coming from the parent (route).
  useEffect(() => {
    if (selectedIndex == null) return;
    if (selectedIndex !== selectedRef.current) syncTo(selectedIndex);
