...
/**
 * MetallicPaint — React Bits liquid-metal shader (WebGL2), ported to TSX.
 *
 * Renders a black-filled silhouette (see /logo.svg) as animated liquid metal.
 * The source image is turned into a signed-distance depth map on the CPU
 * (Gauss–Seidel relaxation) once, then fed to the fragment shader.
 *
 * Two local additions over the upstream component:
 *   - a module-level cache of the processed ImageData keyed by imageSrc, so the
 *     expensive CPU pass runs only once even when several instances (boot
 *     screen, per-route buffers, header) share the same logo;
 *   - a `resolution` prop, so a small header logo needn't render at 1000px².
 */
...
/** Processed-depth cache — the CPU pass depends only on the image, not props. */
...
export interface MetallicPaintProps {
  ...
  contour?: number;
  tintColor?: string;
  /** Internal square render resolution in CSS px (× dpr). Smaller = cheaper. */
  resolution?: number;
  className?: string;
}
...
export default function MetallicPaint({
  ...
  useEffect(() => {
    ...
    const canvas = canvasRef.current!;
    const gl = glRef.current!;
    const side = Math.round(resolution * (typeof devicePixelRatio === "number" ? devicePixelRatio : 1));
    canvas.width = side;
    canvas.height = side;
  ...
  useEffect(() => {
    ...
    // Fast path: reuse a previously processed depth map for this image.
